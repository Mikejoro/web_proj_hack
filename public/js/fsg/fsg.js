...
Fire.prototype.move = function()
{
	...
	else
	{
		...
		for(var i = 0; i < surrounding.length; i++)
		{
			...
			if (temp.flammable && Math.random() > .9 && temp.y < this.y)
			{
				var nFire = new Fire(temp.x, temp.y);
				pixArray[temp.x][temp.y] = nFire;
				particleList.push(nFire);
				temp.x = -1;
			}
			else if (temp.flammable && Math.random() > .8 && temp.y == this.y)
			{
				var nFire = new Fire(temp.x, temp.y);
				pixArray[temp.x][temp.y] = nFire;
				particleList.push(nFire);
			...
			else if (temp.flammable && Math.random() > .333 && temp.y > this.y)
			{
				var nFire = new Fire(temp.x, temp.y);
				pixArray[temp.x][temp.y] = nFire;
				particleList.push(nFire);
			...
			else if(temp.type !== "Water" && !temp.fireProof && Math.random() > .95)
			{
...
var TypeEnum = Object.freeze(
{
	...
	"Wax": 4,
	"Fire": 5,
	"KeyLookup": ["Wall", "Sand", "Water", "Acid", "Wax", "Fire"]
});
...
{	
	for (var i = 0 - brushSize; i < 0; i++)
	{
		for (var n = 0 - brushSize; n < 0; n++)
		{ 
			if(!pixArray[x + i][y + n] && (/* i % 3 == 0 || n % 3 == 0 */ Math.random() > .333 || type === TypeEnum.Wall))
			{
				var temp;
...
{ 
	...
	for (var i = 0; i < particleList.length; i++)
	{
		...
		if(tempX > canvas.width + 1 || tempX < 0
			|| tempY > canvas.height + 1 || tempY < 0)
		{
